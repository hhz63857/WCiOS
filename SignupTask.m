...
-(instancetype)initWithUsername:(NSString *)username pwd:(NSString *)pwd
{
    ...
    if (username && pwd) {
        un = username;
        p = pwd;
        self.url = [NSString stringWithFormat:NW_SING_UP_HTTP_GET_FORMAT, NW_SERVERHOST, NW_SING_UP_PATH, [self _encode:username], [self _encode:pwd]];
    }
    return self;
}
