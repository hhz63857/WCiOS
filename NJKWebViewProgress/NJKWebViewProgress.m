...
- (id)initWithParentVC:(WebViewController *)parentVC
{
...
{
    if ([self.parentVC respondsToSelector:@selector(preWebViewDidFinishLoad)]) {
        [self.parentVC preWebViewDidFinishLoad];
    }

    //    ((WebViewController *)self.parentVC).urlTextField.text = [webView.request.mainDocumentURL absoluteString];
